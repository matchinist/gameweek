...
// Canonical betting semantics (Phase 3.2, owner-ratified 2026-08-31): every
// configured market scores via marketActual — winner and bucket markets
// included. score-round stores THIS; buildBettingEventHistory above stays
// as the legacy client path until the 3.5 cutover retires it.
export function scoreBettingEventAllMarkets(comp, ev, val){
  let pts=0; const perMarket=[];
  (comp.markets||[]).forEach(mkt=>{
    const pick=(val&&val[mkt.type])||null;
    let isRight=false, mPts=0;
    if(ev.res){
      const actual=marketActual(mkt, ev.res);
      isRight = !!pick && actual!=null && pick===actual;
      mPts = isRight?(mkt.points||0):0;
      pts += mPts;
    }
    perMarket.push({type:mkt.type,pick,isRight,pts:mPts});
  });
  return {pts, perMarket};
}
