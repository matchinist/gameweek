...
# get_customer_public(client_key) — same nine safe columns, but you must
...
docker exec -i "$CONTAINER" psql -q -U postgres -v ON_ERROR_STOP=1 <<'SQL'
-- post-chain seed: the rename migration (20260904010000) has already run,
-- so the table and accessor carry their customer-era names here.
insert into gw_customers (client_key, company_name, email, language, accent_color, sso_secret, domains)
...
check "authenticated cannot dump it either" \
  "$(docker exec "$CONTAINER" psql -U postgres -qtA -v ON_ERROR_STOP=1 -c "begin; set local role authenticated; select count(*) from gw_customers_public; rollback;" >/dev/null 2>&1 && echo OK || echo ERR)" "ERR"
check "anon fetches one known client through the accessor" \
  "$(as_anon "select company_name || ':' || language || ':' || accent_color from get_customer_public('acme_x1');")" "Acme FC:tr:#123456"
